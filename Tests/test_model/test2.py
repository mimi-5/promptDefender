# -*- coding: utf-8 -*-
"""
PromptDefender — Test Couche 2 (ML/BERT)
=========================================
Teste la couche 2 seule sur le dataset local JSONL.

Usage :
    python Tests/test_model/test2.py
    python Tests/test_model/test2.py --dataset Tests/data/Prompt_INJECTION_And_Benign_DATASET.jsonl
    python Tests/test_model/test2.py --save-errors
    python Tests/test_model/test2.py --threshold 0.6
"""
...
DEFAULT_ERRORS  = "Tests/errors_layer2.jsonl"


def main():
    parser = argparse.ArgumentParser(description="Test Couche 2 — ML/BERT")
    parser.add_argument("--dataset",     default=DEFAULT_DATASET)
    parser.add_argument("--errors-out",  default=DEFAULT_ERRORS)
    parser.add_argument("--save-errors", action="store_true",
                        help="Sauvegarde les erreurs dans errors_layer2.jsonl")
    parser.add_argument("--threshold",   type=float, default=None,
                        help="Override du threshold (defaut : celui de model_meta.json)")
    args = parser.parse_args()

    # ── Chargement du detecteur ───────────────────────────────────────────────
    print("\nChargement du modele ML/BERT...")
    try:
        detector = MLDetector()
        if args.threshold is not None:
            detector.threshold = args.threshold
        print(f"Modele pret — device={detector.device} | threshold={detector.threshold}")
        print(f"Classificateur : {detector.model_name} | scaling={detector.needs_scaling}")
    ...
    for item in tqdm(samples, desc="Layer 2"):
        prompt     = item["prompt"]
    ...
    print(f"\n{'='*55}")
    print(f"  RESULTATS — Couche 2 (ML/BERT)")
    print(f"{'='*55}")
    print(f"  Classificateur    : {detector.model_name}")
    ...
    print(f"  {'L2 — ML/BERT (seule)':<30} {precision:>10.4f} {recall:>8.4f} {f1:>8.4f}")
    print(f"{'='*55}\n")
